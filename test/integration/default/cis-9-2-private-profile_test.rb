...
# Inspec test for recipe cb_cis_windows_2016::cis-9-2-private-profile
...
control '9.2.5' do
  impact 1.0
  title 'Ensure Windows Firewall: Private: Settings: Apply local firewall rules is set to Yes (default)'
  desc 'Ensure Windows Firewall: Private: Settings: Apply local firewall rules is set to Yes (default)'
  tag 'cis-level-1', 'cis-9.2.5'
  ref 'CIS Windows 2016 RTM (Release 1607) v1.0.0', url: 'https://www.cisecurity.org/cis-benchmarks/'
...
control '9.2.6' do
  impact 1.0
  title 'Ensure Windows Firewall: Private: Settings: Apply local connection security rules is set to Yes (default)'
  desc 'Ensure Windows Firewall: Private: Settings: Apply local connection security rules is set to Yes (default)'
  tag 'cis-level-1', 'cis-9.2.6'
  ref 'CIS Windows 2016 RTM (Release 1607) v1.0.0', url: 'https://www.cisecurity.org/cis-benchmarks/'
...
control '9.2.7' do
  ...
  tag 'cis-level-1', 'cis-9.2.7'
  ref 'CIS Windows 2016 RTM (Release 1607) v1.0.0', url: 'https://www.cisecurity.org/cis-benchmarks/'

  describe registry_key('HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\WindowsFirewall\PrivateProfile\Logging') do
    it { should exist }
    it { should have_property_value('LogFilePath ', :type_string, '%SYSTEMROOT%\System32\logfiles\firewall\privatefw.log') }
  end
end
