...
control '18.4.14.1' do
  ...
  describe registry_key('HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\NetworkProvider\HardenedPaths') do
    it { should exist }
    it { should have_property_value('\\*\NETLOGON', :type_dword, 'RequireMutualAuthentication=1,RequireIntegrity=1' }
    it { should have_property_value('\\*\SYSVOL', :type_dword, 'RequireMutualAuthentication=1,RequireIntegrity=1' }
  end
end
